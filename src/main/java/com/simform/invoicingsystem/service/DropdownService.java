...
public class DropdownService {
    ...
    public List<String> getCsm() {
        return csmRepository.findAll().stream().map(Csm::getName).collect(Collectors.toList());
    }

    public List<String> getSalesPerson() {
        return salesPersonRepository.findAll().stream().map(SalesPerson::getName).collect(Collectors.toList());
    }

    public List<String> getLeadSource() {
        return leadSourceRepository.findAll().stream().map(LeadSource::getSource).collect(Collectors.toList());
    }

    public List<String> getMarketingChannel() {
        return marketingChannelRepository.findAll().stream().map(MarketingChannel::getChannel).collect(Collectors.toList());
    }

    public List<String> getProjectModel() {
        return projectModelRepository.findAll().stream().map(ProjectModel::getModel).collect(Collectors.toList());
    }

    public List<String> getInvoiceCycle() {
        return invoiceCycleRepository.findAll().stream().map(InvoiceCycle::getCycle).collect(Collectors.toList());
    }

    public List<String> getAccType() {
        return accTypeRepository.findAll().stream().map(AccType::getAccType).collect(Collectors.toList());
    }
